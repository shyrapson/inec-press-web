      ...
      <div className="flex flex-col gap-1">
        <div className="flex justify-between gap-2">
          {Array.from({ length: 5 }).map((_, index) => {
            const isActive =
              index + 1 <= currentStep ? "bg-primary" : " bg-gray-1";
            return (
              <div key={index} className={cn("h-3 w-full", isActive)}></div>
            );
          })}
        </div>
        <div className="flex items-start justify-between mt-3">
          ...
          <p className="text-text-1 font-medium text-lg">
            Step {currentStep}/5
          </p>
        </div>
      </div>
